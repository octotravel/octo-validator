...
//       .required(),
//     deliveryMethods: yup
//       .array(yup.mixed().oneOf(Object.values(DeliveryMethod),{message:'Atleast one is required'}).required())
//       .min(1)
//       .ensure(),
//   });
  ...
  .shape({
    ...
    productTimes: yup.array().nullable(true).required('Atleast one is required'),
    capabilities: yup.array().nullable(true).required('Atleast one is required'),
    supplierId: yup.string().required('This field is required'),
    // productStartTimes: yup.object().when('productTimes',{
    //     is:(val:any) => {
    //         return val?.includes("productStartTimes")
    //       },
    //     then:productSchema.nullable(true).defined()
    // }),
